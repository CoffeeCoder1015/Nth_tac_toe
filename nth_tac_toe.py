...
class Game:
    ...
    def display(self,mode="Parsed"):
        if mode.lower() == "plain":
            print(self.gameBoard)
        if mode.lower() == "parsed":
            xy_row = [i+1 for i in range(0,self.boardSize)]
            print("/ "+str(xy_row).replace("[","").replace("]","").replace(",",""))
            for i in range(0,self.boardSize):
                itms = str(self.gameBoard[i]).replace("[","").replace("]","").replace("'","")
                print(i+1,itms)
        self.check()